#ifndef BOX2D_PHYSICS_SERVER_2D_H
#define BOX2D_PHYSICS_SERVER_2D_H
...
class Box2DPhysicsServer2D : public PhysicsServer2DExtension {
	...
	HashMap<uint32_t, Box2DSpace2D *> active_spaces;

	mutable RID_PtrOwner<Box2DShape2D, true> shape_owner;
	mutable RID_PtrOwner<Box2DSpace2D, true> space_owner;
	mutable RID_PtrOwner<Box2DArea2D, true> area_owner;
	mutable RID_PtrOwner<Box2DBody2D, true> body_owner;
	mutable RID_PtrOwner<Box2DJoint2D, true> joint_owner;
...
public:
	...
	Box2DSpace2D *get_active_space(box2d::Handle p_handle) const {
		ERR_FAIL_COND_V(!box2d::is_handle_valid(p_handle), nullptr);
		return active_spaces.get(box2d::handle_hash(p_handle));
	}

	Box2DPhysicsServer2D(bool p_using_threads = false);
	~Box2DPhysicsServer2D() {}
};

class Box2DPhysicsServer2DFactory : public Object {
	GDCLASS(Box2DPhysicsServer2DFactory, Object);

protected:
	static void _bind_methods() {
		ClassDB::bind_method(D_METHOD("create_box2d_callback"), &Box2DPhysicsServer2DFactory::_create_box2d_callback);
	}

public:
	PhysicsServer2D *_create_box2d_callback() {
		PhysicsServer2D *physics_server_2d = memnew(Box2DPhysicsServer2D());
		return physics_server_2d;
	}
};

#endif // BOX2D_PHYSICS_SERVER_2D_H
