...
public:
	...
	static Vector<SweepTestResult> shape_cast(SweepShape p_sweep_shape_A, b2Shape *shape_A, SweepShape p_sweep_shape_B, b2Shape *shape_B, float margin);
	...
	static Vector<b2Fixture *> query_aabb_motion(Vector<Box2DCollisionObject::Shape> p_shapes, const Transform2D &p_transform, const Vector2 &p_motion, double p_margin, uint32_t p_collision_layer, uint32_t p_collision_mask, bool p_collide_with_bodies, bool p_collide_with_areas, Box2DDirectSpaceState *space_state);
	...
	static Vector<SweepTestResult> closest_result_in_cast(Vector<SweepTestResult> p_results);
};
