#ifndef BOX2D_COLLISION_OBJECT_2D_H
#define BOX2D_COLLISION_OBJECT_2D_H
...
class Box2DCollisionObject2D : public Box2DShapeOwner2D {
public:
	enum Type {
		TYPE_AREA,
		TYPE_BODY
...
private:
	...
	struct Shape {
		...
		box2d::Handle collider_handle = box2d::invalid_handle();
	};
...
protected:
	...
	box2d::Handle body_handle = box2d::invalid_handle();
	...
	virtual void _init_collider(box2d::Handle collider_handle) const {}

	virtual void _shapes_changed() = 0;
	void _set_space(Box2DSpace2D *p_space);

	Box2DCollisionObject2D(Type p_type);
...
public:
	...
	_FORCE_INLINE_ box2d::Handle get_body_handle() { return body_handle; }
	...
	void set_body_user_data(box2d::UserData &r_user_data) const;
	static Box2DCollisionObject2D *get_body_user_data(const box2d::UserData &p_user_data);

	void set_collider_user_data(box2d::UserData &r_user_data, uint32_t p_shape_index) const;
	static Box2DCollisionObject2D *get_collider_user_data(const box2d::UserData &p_user_data, uint32_t &r_shape_index);

	void _shape_changed(Box2DShape2D *p_shape) override;

	_FORCE_INLINE_ Type get_type() const { return type; }
	void add_shape(Box2DShape2D *p_shape, const Transform2D &p_transform = Transform2D(), bool p_disabled = false);
	void set_shape(int p_index, Box2DShape2D *p_shape);
	void set_shape_transform(int p_index, const Transform2D &p_transform);

	_FORCE_INLINE_ int get_shape_count() const { return shapes.size(); }

	_FORCE_INLINE_ Box2DShape2D *get_shape(int p_index) const {
	...
	_FORCE_INLINE_ Box2DSpace2D *get_space() const { return space; }
	...
	void remove_shape(Box2DShape2D *p_shape) override;
	void remove_shape(int p_index);

	virtual void set_space(Box2DSpace2D *p_space) = 0;

	void set_pickable(bool p_pickable) { pickable = p_pickable; }
	_FORCE_INLINE_ bool is_pickable() const { return pickable; }

	_FORCE_INLINE_ bool collides_with(Box2DCollisionObject2D *p_other) const {
		return p_other->collision_layer & collision_mask;
	}

	_FORCE_INLINE_ bool interacts_with(const Box2DCollisionObject2D *p_other) const {
		return collision_layer & p_other->collision_mask || p_other->collision_layer & collision_mask;
	}

	virtual ~Box2DCollisionObject2D() {}
};

#endif // BOX2D_COLLISION_OBJECT_2D_H
