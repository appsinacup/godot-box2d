...
void Box2DArea2D::set_space(Box2DSpace2D *p_space) {
	...
	if (get_space()) {
		...
		for (auto E = detected_bodies.begin(); E != detected_bodies.end(); ++E) {
			Box2DBody2D *body = get_space()->get_body_from_rid(E->key);
...
void Box2DArea2D::on_body_enter(box2d::Handle p_collider_handle, Box2DBody2D *p_body, uint32_t p_body_shape, RID p_body_rid, ObjectID p_body_instance_id, box2d::Handle p_area_collider_handle, uint32_t p_area_shape) {
	...
	uint64_t handle_pair_hash = box2d::handle_pair_hash(p_collider_handle, p_area_collider_handle);
...
void Box2DArea2D::on_body_exit(box2d::Handle p_collider_handle, Box2DBody2D *p_body, uint32_t p_body_shape, RID p_body_rid, ObjectID p_body_instance_id, box2d::Handle p_area_collider_handle, uint32_t p_area_shape, bool p_update_detection) {
...
void Box2DArea2D::on_area_enter(box2d::Handle p_collider_handle, Box2DArea2D *p_other_area, uint32_t p_other_area_shape, RID p_other_area_rid, ObjectID p_other_area_instance_id, box2d::Handle p_area_collider_handle, uint32_t p_area_shape) {
...
void Box2DArea2D::on_area_exit(box2d::Handle p_collider_handle, Box2DArea2D *p_other_area, uint32_t p_other_area_shape, RID p_other_area_rid, ObjectID p_other_area_instance_id, box2d::Handle p_area_collider_handle, uint32_t p_area_shape) {
	if (area_monitor_callback.is_null()) {
		return;
	}
...
void Box2DArea2D::update_area_override() {
	area_override_update_list.remove_from_list();
...
void Box2DArea2D::set_monitor_callback(const Callable &p_callback) {
...
void Box2DArea2D::set_area_monitor_callback(const Callable &p_callback) {
	uint64_t id = p_callback.get_object_id();
...
void Box2DArea2D::_set_space_override_mode(PhysicsServer2D::AreaSpaceOverrideMode &r_mode, PhysicsServer2D::AreaSpaceOverrideMode p_value) {
...
bool Box2DArea2D::has_any_space_override() const {
...
void Box2DArea2D::_enable_space_override() {
	for (auto E = detected_bodies.begin(); E != detected_bodies.end(); ++E) {
		Box2DBody2D *body = get_space()->get_body_from_rid(E->key);
		if (!body || !body->get_space()) {
			continue;
...
void Box2DArea2D::_disable_space_override() {
	for (auto E = detected_bodies.begin(); E != detected_bodies.end(); ++E) {
		Box2DBody2D *body = get_space()->get_body_from_rid(E->key);
		if (!body || !body->get_space()) {
			continue;
...
void Box2DArea2D::_reset_space_override() {
	for (auto E = detected_bodies.begin(); E != detected_bodies.end(); ++E) {
		Box2DBody2D *body = get_space()->get_body_from_rid(E->key);
		if (!body || !body->get_space()) {
			continue;
...
void Box2DArea2D::set_param(PhysicsServer2D::AreaParameter p_param, const Variant &p_value) {
	switch (p_param) {
...
Variant Box2DArea2D::get_param(PhysicsServer2D::AreaParameter p_param) const {
	switch (p_param) {
		case PhysicsServer2D::AREA_PARAM_GRAVITY_OVERRIDE_MODE:
...
void Box2DArea2D::compute_gravity(const Vector2 &p_position, Vector2 &r_gravity) const {
	if (is_gravity_point()) {
