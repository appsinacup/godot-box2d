#ifndef BOX2D_BODY_2D_H
#define BOX2D_BODY_2D_H
...
class Box2DBody2D : public Box2DCollisionObject2D {
	...
	SelfList<Box2DBody2D> active_list;
	SelfList<Box2DBody2D> mass_properties_update_list;
	SelfList<Box2DBody2D> gravity_update_list;

	//List<Pair<Box2DConstraint2D *, int>> constraint_list;

	struct AreaCMP {
		Box2DArea2D *area = nullptr;
		_FORCE_INLINE_ bool operator==(const AreaCMP &p_cmp) const { return area->get_rid() == p_cmp.area->get_rid(); }
		_FORCE_INLINE_ bool operator<(const AreaCMP &p_cmp) const { return area->get_priority() < p_cmp.area->get_priority(); }
		_FORCE_INLINE_ AreaCMP() {}
		_FORCE_INLINE_ AreaCMP(Box2DArea2D *p_area) {
			area = p_area;
		}
	};

	LocalVector<AreaCMP> areas;
	SelfList<Box2DBody2D> area_override_update_list;
	...
	SelfList<Box2DBody2D> direct_state_query_list;

	friend class Box2DDirectBodyState2D; // i give up, too many functions to expose
...
protected:
	...
	virtual void _init_collider(box2d::Handle collider_handle) const override;
...
public:
	...
	void add_area(Box2DArea2D *p_area);
	void remove_area(Box2DArea2D *p_area);
	void on_area_updated(Box2DArea2D *p_area);
...
void Box2DBody2D::add_contact(const Vector2 &p_local_pos, const Vector2 &p_local_normal, real_t p_depth, int p_local_shape, const Vector2 &p_collider_pos, int p_collider_shape, ObjectID p_collider_instance_id, const RID &p_collider, const Vector2 &p_collider_velocity_at_pos, const Vector2 &p_impulse) {
